...
from utils.recon_helpers import setup_camera, setup_camera_light
...
from utils.slam_helpers import (
    ...
    quat_mult, matrix_to_quaternion, rendervar_raster_settings, transformed_params2rendervar_light
)
...
def plot_rgbd_silhouette(color, depth, rastered_color, rastered_depth, presence_sil_mask, diff_depth_l1,
    ...
    axs[1, 1].imshow(rastered_depth.cpu(), cmap='jet', vmin=0, vmax=6)
...
def eval(dataset, final_params, num_frames, eval_dir, sil_thres, 
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        valid_depth_mask = (curr_data['depth'] > 0)
        rastered_depth_viz = rastered_depth.detach().squeeze(0)
...
def eval_light(dataset, final_params, num_frames, eval_dir, sil_thres, 
         mapping_iters, add_new_gaussians, wandb_run=None, wandb_save_qual=False, eval_every=1, save_frames=False):
    print("Evaluating Final Parameters ...")
    psnr_list = []
    rmse_list = []
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        color = color.permute(2, 0, 1) / 255 # (H, W, C) -> (C, H, W)
        depth = depth.permute(2, 0, 1) # (H, W, C) -> (C, H, W)
        curr_data = {'im': color, 'depth': depth, 'intrinsics' : intrinsics}
        raster_settings, rendervar_light = rendervar_raster_settings(curr_data, final_params, time_idx, False, False)
        im, _, rastered_depth, _, _, silhouette, *rest = RendererLight(raster_settings)(**rendervar_light)
